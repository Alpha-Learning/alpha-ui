...
export default function KS2InterviewQuestionsPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (response.success) {
        ...
        clearStorage();
        router.push(
          `/form/questionnaire-success?type=ks2-interview&applicationId=${params.id}`
        );
      } else {
        setMessage({
          type: "error",
          text: response.message || "Failed to submit form",
        });
      }
    } catch (error: any) {
      setMessage({
  ...
  const handleAutofill = () => {
    const autofillData = getAutofillData('ks2Interview');
