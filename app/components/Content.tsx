    ...
    <div className="absolute  left-0 right-0 sm:left-8 md:left-49 top-16 sm:top-20 md:top-auto lg:top-auto xl:top-auto bottom-2 sm:bottom-4 md:bottom-6 lg:bottom-8 xl:bottom-10 px-4 sm:px-6 md:px-8 lg:px-10 content-return-up overflow-y-auto scroll-invisible ">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 md:gap-10 lg:gap-6">
        {[
          ...
          const animationDelayClass = i === 0 ? '' : i === 1 ? '[animation-delay:200ms]' : '[animation-delay:400ms]';
          const borderDelayClass = i === 0 ? 'delay-[0ms]' : i === 1 ? 'delay-[160ms]' : 'delay-[320ms]';
          const contentDelayClass = i === 0 ? 'delay-[60ms]' : i === 1 ? 'delay-[220ms]' : 'delay-[380ms]';
          return (
          <div key={i} 
          className={`relative bg-white/40 backdrop-blur-sm rounded-tr-4xl min-h-[180px] sm:min-h-[200px] md:min-h-[220px] lg:min-h-[240px] w-full max-w-sm mx-auto sm:mx-0 shadow-lg  slide-up-slow ${animationDelayClass}`}
          >
            {/* Connected border overlay that reveals sequentially */}
            <div
              className={`absolute inset-0 z-10 pointer-events-none transition-all duration-700 ease-out ${borderDelayClass} rounded-tr-2xl md:rounded-tr-4xl border-t border-r ${
                i < borderStep ? "opacity-100 border-gray-200" : "opacity-0 border-transparent"
              }`}
            />
            <div
              className={`relative z-0 text-[#142954] flex flex-col gap-y-2 sm:gap-y-4 md:gap-y-6 lg:gap-y-8 xl:gap-y-0 justify-between p-3 sm:p-4 md:p-5 pt-2 sm:pt-3 transition-all duration-700 ease-out ${contentDelayClass} ${
