...
export async function sendPaymentEmail(data: PaymentEmailData): Promise<boolean> {
  try {
    ...
    // Use additionalNotes for the actual walkthrough date
    const formattedWalkthroughDate = formatDate(data.walkthroughDate);
    const formattedWalkthroughTime = formatTime(data.walkthroughDate);
    const hasWalkthroughDate = formattedWalkthroughDate && formattedWalkthroughTime;
    
    const mailOptions = {
      from: `"Alphera Academy" <${emailConfig.auth.user}>`,
      to: data.parentEmail,
      subject: `Screening Call Completed - Facility Walkthrough Scheduled - Alphera Academy`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="utf-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Screening Call Completed - Alphera Academy</title>
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
            .container { max-width: 600px; margin: 0 auto; padding: 20px; }
            .header { background: linear-gradient(135deg, #8EC0C2, #142954); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
            .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
            .info-box { background: white; border: 2px solid #8EC0C2; border-radius: 8px; padding: 25px; margin: 20px 0; }
            .details-table { width: 100%; border-collapse: collapse; margin: 20px 0; }
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1>🎓 Alphera Academy</h1>
              <h2>Screening Call Completed</h2>
            </div>
            
            <div class="content">
              <p>Dear ${data.parentName},</p>
              
              <p>Thank you for completing the screening call for your child <strong>${data.childName}</strong>.</p>
              
              ${hasWalkthroughDate ? `
              <div class="info-box">
                <h3 style="color: #142954; margin-top: 0; font-size: 20px;">🏫 Facility Walkthrough Scheduled</h3>
                <table class="details-table">
                  <tr>
                    <td>Date:</td>
                    <td><strong>${formattedWalkthroughDate}</strong></td>
                  </tr>
                  <tr>
                    <td>Time:</td>
                    <td><strong>${formattedWalkthroughTime}</strong></td>
                  </tr>
                </table>
                <p style="margin-top: 20px; color: #142954; font-weight: bold;">Please arrive at the scheduled date and time mentioned above.</p>
              </div>
              ` : ''}
      ...
      text: `
        ...
        ${hasWalkthroughDate ? `
        FACILITY WALKTHROUGH SCHEDULED
        
        Date: ${formattedWalkthroughDate}
        Time: ${formattedWalkthroughTime}
        
        Please arrive at the scheduled date and time mentioned above.
        ` : ''}
